...
A1 已完成**阅读选区到原始 Markdown 字节范围的映射原型**，覆盖普通文本、重复文字、加粗/链接、中文、emoji、BOM 和 CRLF。A2 已接入受限 YAML sidecar、源摘要与磁盘冲突校验、恢复草稿和最小测试锚点保存入口。下一批按用户试用反馈推进 A3 混排排版，再逐批完成高亮、便签与标签，同时保留章节移动、画布和全图导出的 P0 门槛。
