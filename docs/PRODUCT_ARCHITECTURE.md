...
打开相同摘要的原文时，先核验存储字节范围及原文片段；摘要不同或发生应用内编辑、章节移动时，用文本补丁映射、上下文、章节线索做保守的一对一重定位。只接受唯一高置信候选；删除、重复文本或歧义使批注进入“待定位”，保留便签内容并提供人工重新选区，绝不静默贴到别处。在自动重定位切片完成前，摘要不符的旧锚点一律先待定位，绝不按旧偏移绘制。正文标记和跳转目标每次由核验后的 DOM Range 计算；边栏卡片不保存或依赖像素坐标。批注正文和标签名均作为纯文本渲染，不作为 HTML 执行。
...
A1 已完成**阅读选区到原始 Markdown 字节范围的映射原型**，覆盖普通文本、重复文字、加粗/链接、中文、emoji、BOM 和 CRLF。A2 已接入受限 YAML sidecar、源摘要与磁盘冲突校验及恢复草稿。A3 已完成语义表格、列表和代码块的混排布局，并在 Windows 打包版的不同窗口及实际缩放下验收。A4 已完成选区高亮、改色、删除与重开恢复。A5 已完成便签新增、编辑、删除，单标签选择与创建、标签筛选、原文跳转，以及宽屏边栏和窄屏抽屉。下一批等待用户试用反馈后推进 A6 保守重定位、人工重选和阅读摘要；章节移动、画布和全图导出的 P0 门槛仍保留。
