# MerMarkd 产品设计与技术架构（v0.4）

> 2026-09-17，实施状态更新至 2026-09-24。本文是新项目的实施蓝图。默认首发 Windows 桌面版、单个本地 Markdown 文件、离线工作；macOS/Linux 作为后续平台。估期按一名熟练的全栈开发者计算。开发门槛与未验证约束见[可行性审查](FEASIBILITY_REVIEW.md)，实施规则见[开发流程](DEVELOPMENT_PROCESS.md)，三模式的完整职责见[三模式职责与交互边界](MODE_BOUNDARIES.md)。
...
4. **只有明确改变卡片容纳关系才改 Markdown 结构。** 把章节放入另一卡片或明确移出为更高层级时，移动整个章节子树，并重算被移动子树的标题级别；正文字符、代码、图片路径等保持原样。编辑模式中的输入先改缓冲区，只有显式保存才修改 `.md`。对无法证明语义安全的移动，应用阻止操作。
...
| --- | --- | --- | --- |
| 编辑模式输入、粘贴 | 未保存前只改内存缓冲区；显式保存后修改 | dirty 期间暂停依赖新基线的写入 | dirty 期间暂停依赖新结构的写入 |
| 阅读、查找、模式切换 | 不修改 | 不修改 | 不修改 |
...
| 模式 | 核心界面 | 操作 | 文件影响 |
| --- | --- | --- | --- |
| 编辑模式 | 纯源码编辑器、源码目录、行列状态 | 输入、源码查找替换、跳转、撤销/重做、显式保存 | 输入先改内存缓冲区；保存才改变 `.md`；不渲染正文 |
| Markdown 阅读模式 | 语义化排版正文、目录、批注边栏与标签筛选 | 阅读、可见正文查找、目录跳转、选中文字高亮或添加批注、复制摘要 | `.md` 只读；批注 sidecar 可修改；不显示受支持结构的源码语法 |
| 卡片模式 | 可缩放画布、章节卡片、箭头、属性面板 | 拖动布局、连线、折叠、结构拖入、完整场景导出 | 除明确确认的结构变更外只改画布数据 |

打开或拖入文件、文件名、外部变化、未保存/冲突状态和三模式切换属于共同外壳。搜索入口按模式分派；阅读摘要只在阅读模式，PDF/JPG/PNG 全图导出只在卡片模式。模式切换本身不写 `.md`、批注 YAML 或画布 JSON。三种模式共享一个 `DocumentSession` 和当前章节；点击阅读标题后切到卡片模式应定位同一章节，卡片上的“编辑本节”应跳到对应源码位置。

阅读模式对 CommonMark + GFM 结构只呈现语义结果：标题不显示 `#`，强调不显示定界符，链接不显示目标语法，表格不显示管线与对齐横线，代码块显示代码内容但不显示围栏。frontmatter 作为元数据从正文隐藏；原始 HTML 不经未清洗路径进入 DOM，允许列表内的语义内容可安全呈现，其余节点显示明确占位，危险容器内容不作为普通正文提取。只有能精确映射到源码的 HTML 派生文字才允许批注。对能够无歧义识别的已知扩展给出文档级提示，其他内容按当前方言的普通文本处理，不承诺任意 Markdown 都能无语法渲染。`H1/H2` 不进入阅读界面；UTF-8 字节范围和 `sourceExact` 仅进入开发诊断；SHA 和 sidecar 路径只进入用户主动打开的文档信息。

阅读视图选中正文后即时核验并出现轻量操作条：显示可见引文或按 Unicode 字素计算的字符数，提供高亮色和“添加批注”。无选区时不常驻整排操作；无法可靠映射的跨块或复杂选区保留选择并说明原因，拒绝保存且不展示源码或字节偏移。操作条取得焦点后保留已经核验的目标范围，不从全文重新搜索同名文字。`Escape` 清除普通选区；人工重选期间则取消重选且不写 sidecar。正文滚动、目录跳转和文内标题跳转同步当前章节；当前模式、章节、批注与选区均使用非纯颜色反馈和相应 ARIA 状态。

添加批注时在右侧边栏展开便签，输入纯文本批注，并在创建时选择已有标签、创建一个标签或选择“无标签”；保存后可按标签筛选、跳回原文、编辑或删除。高亮可随后转成带批注的同一条记录，避免重复锚点。首版只允许选区完全位于正文的一份文档中；后续覆盖标题、普通段落、列表、表格单元格和代码块等已验证类型。编辑模式的源码选区不提供阅读批注工具。

批注边栏按标签筛选，并提供“复制阅读摘要”。A6 的首版摘要沿用当前“全部 / 无标签 / 指定标签”筛选：已定位记录按当前原文 UTF-8 字节位置排序，再按当前章节路径组织；没有章节的已定位记录单独成组；待定位记录按 sidecar 原顺序附在末尾并保留原章节线索。输出包含引文、标签、高亮颜色和便签正文，用户文本先转义为纯 Markdown 引用，再写入系统剪贴板。生成摘要不修改 `.md` 或批注 sidecar，也不把便签正文混入章节卡片原文；另存摘要文件仍属后续能力。

“内容优先”的默认布局为稳定正文行宽、低饱和度高亮和窄边栏。便签在边栏中按普通文档流纵向排列，当前选中项展开全文，其余项显示摘要，不互相遮挡或压在正文上。宽窗口的边栏进入页面正常布局；宽度不超过 1100 px 时改用可收起的模态抽屉，支持焦点循环、Escape 关闭并把焦点还给入口按钮。标签只在边栏出现，不把彩色徽章插入每段正文。重叠高亮以当前选中项优先呈现，不叠加不透明颜色；键盘和屏幕阅读器可以定位批注。

表格与列表的视觉位置由 HTML/CSS 布局决定，不能按字符数、全角/半角比例或补空格推算。GFM 表格保留语义 `<table>` 与单元格对齐，外层容器在窄窗口横向滚动；列表使用 `<ol>/<ul>/<li>` 标记与一致缩进，任务框在阅读模式只读。代码块和源码编辑器使用合适的等宽字体回退、明确的 tab 宽度及横向滚动。中英文、全半角标点、emoji 和组合字符在不同字体下宽度不恒定；如将来提供源码表格格式化，必须由用户显式触发、预览并可撤销，默认不改原文。
...
以下是 A2 固定的 v1 格式示意；受限解析器限制 sidecar UTF-8 文件为 1 MiB，拒绝未知字段、注释、别名和显式 YAML 标签。高亮和批注共用一条记录：只有高亮时 `kind` 为 `highlight`，添加便签后更新为 `note` 并保留 ID；直接创建便签时 `color` 可省略。首版一条批注可选零个或一个标签，`tagId` 指向稳定标签 ID；“无标签”是明确选项。示例对应 UTF-8、LF、无 BOM 的 `example.md`，其完整内容为 `# 示例\n这是一段关键结论。\n`（其中 `\n` 表示换行）：
...
打开相同摘要的原文时，先核验存储字节范围、原文片段和可见文字。A6 在摘要不同时采用更严格的首版策略：只有 `sourceExact` 在当前完整源码中恰好出现一次、锚点保存的左右上下文均仍精确相邻、候选能反向还原为当前阅读 DOM 的唯一范围，且不会与其他记录落到同一范围时，才标为“可安全重定位”。旧字节位置和 `sectionHint` 不用于破除重复；删除、复制后重复、任一侧上下文变化、非渲染区域或目标碰撞都保持“待定位”。候选只在用户显式应用后写回；未解决记录保留旧 `basisSha256` 和全部内容，因此同一 sidecar 可以包含不同锚点基线。用户也可为待定位记录重新选择当前可精确映射的单块文字，保留记录 ID、创建时间、顺序、便签、标签和颜色。更宽松的文本补丁映射及章节结构移动后的批注映射留待后续切片。正文标记和跳转目标每次由核验后的 DOM Range 计算；边栏卡片不保存或依赖像素坐标。批注正文和标签名均作为纯文本渲染，不作为 HTML 执行。

为了让 Git diff 稳定，sidecar 使用版本化 schema、稳定 ID、固定字段顺序和稳定记录顺序；新记录追加，编辑只更新相关记录，不因打开文件就改写所有时间戳或排序。创建标签时去除首尾空白并做 NFC 规范化；同大小写且 NFC 等价的名称复用已有标签，大小写不同的名称可以并存。新标签与便签或便签更新先形成同一个内存候选，再只保存一次 sidecar；删除便签不自动删除未使用标签。YAML 作为文本格式便于版本控制，**不能保证 Git 自动合并不冲突**。解析时限制文件大小、别名/自定义标签等不可信输入；未知版本、不合法文件或无法保留的未知字段/注释应只读提示，不覆盖原文件。
...
编辑模式维护一份属于当前 `DocumentSession` 的源码缓冲区，不让编辑器、阅读器和卡片视图各自保存会漂移的正文副本。编辑输入只改变缓冲区与单文档恢复草稿；切换到阅读模式可把该缓冲区呈现为明确标记的“未保存预览”，但在保存并完成锚点核验前，暂停批注新增/编辑/重定位和卡片结构写入。卡片模式只可显示同样标记的只读结构预览并允许定位、平移和缩放；布局持久化、连线、结构操作及导出保持禁用，直到 Markdown 保存且画布基线重新核验。打开其他文件、重新载入或退出时，不得丢弃未保存缓冲区或静默保存。无操作保存必须保持原始字节摘要不变。
...
- 单独新增高亮或批注只原子替换批注 YAML，不触碰 `.md`。A6 经过用户审查的旧 sidecar 迁移仍是单文件替换，但保存时同时校验当前 Markdown 摘要、sidecar 字节摘要和迁移前的 `source.sha256`；任一基线变化都拒绝覆盖并把完整候选保存到应用数据目录。需要同时提交结构文本、批注重定位与画布更新时，先记录包含三者预期摘要的恢复事务，再逐文件临时写入与替换；中断后按事务日志恢复或回滚。撤销/重做覆盖批注和结构操作；崩溃恢复草稿存入应用数据目录。
...
| Markdown 管线 | 现有 unified / remark-parse / remark-gfm / remark-frontmatter + react-markdown；按需增加受控渲染扩展 | 统一解析配置与源码位置映射，驱动章节、阅读和批注锚点；frontmatter 单独识别。原始 HTML 不经未清洗路径进入 DOM：允许列表内语义内容受控呈现，其余节点显示占位，危险容器内容不提取。当前阅读实现与核心各自解析，选区原型须验证两者位置一致。 |
...
**切换模式与定位：** 把当前章节和源位置写入会话内的活动位置 → 清理阅读临时选区、浮动操作和不再适用的焦点状态 → 目标模式从同一源码缓冲区和章节树派生界面 → 把焦点送到对应正文、源码行或卡片。切换本身不触发内容文件写入；有未保存源码时，阅读视图标明预览状态并暂停依赖已落盘摘要的 sidecar 写入。

**编辑与保存：** 编辑器显示完整源码并只更新内存缓冲区 → 增量解析用于目录和错误提示，但不把源码渲染成编辑界面 → 保存前核验磁盘 SHA、编码、BOM 和换行策略 → 生成最小文本变化并重新解析候选文档 → 原子替换 Markdown → 对受影响批注和卡片 ID 做保守映射或标为待处理。外部基线变化、解析门槛失败或无法写入时保留完整恢复草稿，不覆盖磁盘文件。
...
| 1. 阅读与批注 | 无源码语法的语义阅读、即时选区/当前章节反馈、中英文混排、高亮、便签、标签筛选、待定位修复、阅读摘要 | 支持的结构不泄露语法；选择有即时反馈；高亮和批注重开后落在正确文字上；只改 YAML，`.md` 摘要不变 |
| 2. 文档编辑 | 完整纯源码编辑、保存、目录定位、行列状态、批注与编辑位置映射 | 编辑界面不渲染正文；UTF-8/BOM/CRLF 往返；未保存内容与 sidecar 一致；外部改动不能静默覆盖 |
...
- 阅读模式对支持的 Markdown 只显示语义结果，不显示 `#`、强调定界符、链接目标、表格管线、代码围栏或源码诊断；编辑模式显示完整源码且不渲染正文。
- 当前模式、阅读选区、当前章节、当前批注、编辑器行列和当前卡片均有清楚且非纯颜色的反馈。
...
A1–A6 的阅读锚点、sidecar、混排、高亮、便签、标签、保守重定位、人工重选和剪贴板摘要已经完成。2026-09-24 的模式边界审查确认当前代码仍只有阅读模式，并发现源码诊断泄露、即时选区反馈和滚动章节反馈尚未收口。下一批是 **A7.1 模式导航外壳与阅读交互收口**；完成并等待试用反馈后，再以 A7.2 收口阅读方言和查找。后续依次证明安全保存、接入纯源码编辑、完成批注映射和章节变换。A8.1 固定画布 schema 与稳定 ID 并验证三文件恢复，A8.2 实现嵌套画布与关系持久化，A8.3 单独实现 PDF/JPG/PNG 全图导出。具体规则与验收见[三模式职责与交互边界](MODE_BOUNDARIES.md)。
