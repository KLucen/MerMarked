...
下一批先验证**阅读选区到原始 Markdown 字节范围的映射**，覆盖普通文本、加粗/链接、中文、emoji、BOM 和 CRLF；只做可检查的映射原型，不写 sidecar。用户验收该批结果后，继续按开发流程逐批完成 YAML 持久化、混排排版、高亮、便签与标签，同时保留章节移动、画布和全图导出的 P0 门槛。
