...
A1 已完成**阅读选区到原始 Markdown 字节范围的映射原型**，覆盖普通文本、重复文字、加粗/链接、中文、emoji、BOM 和 CRLF。A2 已接入受限 YAML sidecar、源摘要与磁盘冲突校验、恢复草稿和最小测试锚点保存入口。A3 已完成语义表格、列表和代码块的混排布局，并在 Windows 打包版的不同窗口及实际缩放下验收。下一批等待用户反馈后推进 A4 正式高亮交互；章节移动、画布和全图导出的 P0 门槛仍保留。
