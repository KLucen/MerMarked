# MerMarkd 架构可行性审查（2026-09-17，状态更新 2026-09-24）
...
本机已检查为 Windows x64，Node 24.20.0、npm 11.19.0、Git 2.45.1 可用；没有 Rust/Cargo 或 MSVC 编译工具。纯 TypeScript/Electron 开发已开始，原生图片模块若引入须额外验证预编译包与安装包加载。P0 安全桌面壳、最小阅读器和测试安装包已运行；当前 79 项测试、类型检查、`npm run package` 与 `npm run make` 通过。A1 已验证单块标题/段落中可精确映射的选区；A2 已验证受限 YAML sidecar 保存、重开、冲突保护与原文摘要不变；A3–A6 已在打包版验证混排、高亮、便签标签、保守重定位、人工重选和剪贴板摘要。当前测试安装包为 `out/make/squirrel.windows/x64/MerMarkd-0.1.0 Setup.exe`（153,850,368 字节）。2026-09-24 的审查确认当前代码仍只有阅读模式，而且阅读页仍会显示源码诊断并缺少即时选区和滚动章节反馈；编辑模式的源码往返与保存 IPC 尚未实现。后续先以 A7.1–A7.2 收口阅读，再分批验证保存、编辑、锚点映射和章节变换；画布模型就绪后才验证三文件恢复。
...
| 阅读渲染完整性 | 中；安全与“无源码语法”之间需逐类验证 | `skipHtml` 可能省略 HTML 内容；数学公式、Wiki Link、directive 等已知未支持扩展会按普通文本显示。阅读页现有 `Hn`、字节范围和 `sourceExact` 也违反内容优先。 | 首版合同限定 CommonMark + GFM + frontmatter；为每类受支持节点做阅读 DOM fixture，隐藏结构标记但保留语义内容。HTML 只呈现允许列表内的语义内容或明确占位；派生文字可精确映射前不能批注。只对能无歧义识别的已知扩展给文档级提示，其余按 CommonMark 普通文本处理。`Hn` 移除，源码范围只进开发诊断。 |
| 源码编辑往返 | 中，最高 Markdown 写入风险；尚未实现 | 编辑器字符串、BOM、CRLF/LF、无尾换行、外部修改和未保存预览可能产生无关 diff，或让批注写到错误摘要。 | 纯源码编辑器共享 `DocumentSession` 缓冲区；保存 IPC 只使用会话路径和 expected SHA；保存前后重解析，保留 BOM/换行及未参与字节。dirty 预览暂停 sidecar 写入。用无操作保存、外部冲突和异常中断样本验收。 |
...
| 批注重定位 | 中，最高误绑风险；A6 Windows 打包版小样本已验证 | 只用字节位置会在外部编辑后漂移；只用引文会碰到重复段落。章节移动也改变字节范围。 | 同摘要仍核验字节片段和可见文字。A6 首版在摘要变化后仅当 `sourceExact` 全文唯一、保存的左右上下文均仍精确相邻、当前阅读范围可反向核验且目标不碰撞时提出候选；不使用旧偏移或章节线索破除重复。用户显式应用候选或人工重新选择；删除、重复、上下文变化、非渲染区域及碰撞继续待定位并保留全部内容。Windows 打包版已验证移动/复制/删除、人工修复和冲突草稿；编辑补丁映射留待 A7.4，结构移动后的映射随 A7.5 验证。 |
...
| `.md` 与两类 sidecar 一致性 | 中，最高持久化风险 | `.md` 明确保存、sidecar 自动保存时，YAML/JSON 可能提前引用未保存的文字或章节；三文件不能原子同时替换；双实例也可能覆盖批注或画布。 | 两类 sidecar 均记录其对应的**已落盘 Markdown 摘要**，批注记录另保留各自 `basisSha256`，允许明确的过期或混合基线但不误用旧范围。未保存正文/结构与相关 sidecar 改动进恢复草稿；跨文件保存校验三个磁盘基线并写事务日志，失败或崩溃后恢复或回滚。双实例/外部写入须冲突提示或另存。 |
...
| 源码保存 | BOM/无 BOM、LF/CRLF、无尾换行、无操作保存、外部修改和 dirty 预览 | 无操作摘要不变；有效修改只产生预期字节差异；外部冲突不覆盖；dirty 期间不写错 sidecar 基线 |
