...
本机已检查为 Windows x64，Node 24.20.0、npm 11.19.0、Git 2.45.1 可用；没有 Rust/Cargo 或 MSVC 编译工具。纯 TypeScript/Electron 开发已开始，原生图片模块若引入须额外验证预编译包与安装包加载。P0 安全桌面壳、最小阅读器和测试安装包已运行；当前 20 项测试通过。A1 已验证单块标题/段落中可精确映射的选区；YAML 保存、复杂选区、结构移动、三文件恢复和导出仍待原型验证。
...
| 中英文混排表格/列表 | 高，但需视觉实测 | 实际字宽由字体回退、字形与缩放决定；把中文当两个半角、按空格补列会在表格/列表中错位。当前阅读 CSS 把 `<table>` 设为 `display:block`，需要改成语义表格与外层滚动容器。 | 阅读表格保留浏览器列布局和 GFM 单元格对齐，列表使用原生 marker 与统一缩进；代码块独立等宽/横向滚动。用混排、全半角标点、emoji、组合字符、长 URL、两位数列表在窄屏及 100%/125%/150% 缩放截图验收，不改 `.md`。 |
...
完成对应原型关口后再锁定批注/画布 sidecar schema、导出单图上限及首版性能指标。目前对 5 MB/1000 标题/200 可见卡片的数字只应视为测试目标，不应当作已验证承诺。
