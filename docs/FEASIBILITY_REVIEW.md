# MerMarkd 架构可行性审查（2026-09-17，状态更新 2026-09-23）
...
本机已检查为 Windows x64，Node 24.20.0、npm 11.19.0、Git 2.45.1 可用；没有 Rust/Cargo 或 MSVC 编译工具。纯 TypeScript/Electron 开发已开始，原生图片模块若引入须额外验证预编译包与安装包加载。P0 安全桌面壳、最小阅读器和测试安装包已运行；当前 79 项测试、类型检查、`npm run package` 与 `npm run make` 通过。A1 已验证单块标题/段落中可精确映射的选区；A2 已验证受限 YAML sidecar 保存、重开、冲突保护与原文摘要不变；A3 已在 Windows 打包版验证混排排版及不同缩放；A4 已在打包版验证高亮创建、改色、删除、重开与过期锚点冻结。A5 已在打包版验证便签增改删、单标签选择与筛选、高亮原位转便签、重启恢复、冲突草稿，以及宽屏边栏和窄屏键盘焦点。A6 已由核心测试和 Windows 打包版 E2E 验证严格候选的显式应用、歧义/删除保留待定位、人工重选、剪贴板阅读摘要、外部 sidecar 冲突草稿和窄窗口抽屉。当前测试安装包为 `out/make/squirrel.windows/x64/MerMarkd-0.1.0 Setup.exe`（153,850,368 字节）。复杂选区、更宽松的文本补丁映射、结构移动、结构变更后的锚点映射、三文件恢复和导出仍待原型验证。
...
| 批注重定位 | 中，最高误绑风险；A6 Windows 打包版小样本已验证 | 只用字节位置会在外部编辑后漂移；只用引文会碰到重复段落。章节移动也改变字节范围。 | 同摘要仍核验字节片段和可见文字。A6 首版在摘要变化后仅当 `sourceExact` 全文唯一、保存的左右上下文均仍精确相邻、当前阅读范围可反向核验且目标不碰撞时提出候选；不使用旧偏移或章节线索破除重复。用户显式应用候选或人工重新选择；删除、重复、上下文变化、非渲染区域及碰撞继续待定位并保留全部内容。Windows 打包版已验证移动/复制/删除、人工修复和冲突草稿；文本补丁映射与结构移动后的批注映射留待 A7。 |
